...
def simulate(subgroup_n=[10000, 10000], mu_0=[0, 0], mu_1=[0, 0], sigma_0=[1, 1], sigma_1=[1, 1],
             rounding_thresh=[.5, .5], eta_sd=[0.1, 0.1], eta_mean=[0, 0], sigma_scale_factor=1, mu_scale_factor=0):

    assert len(mu_1) == len(mu_0) == len(sigma_0) == len(sigma_1), 'num of covariates not consistent'
    assert len(subgroup_n) == len(rounding_thresh) == len(eta_sd) == len(eta_mean), 'num of groups not consistent'

    mu = [mu_0, np.dot(mu_1, mu_scale_factor)]
...
def ABROCAvs_plot(plot_type, vs, r=10, s=0, n=[10000, 10000]):
    """plot_type options are 'default', 'sample_size', 'label_dist', 'cov_means', 'label_noise', 'obs_noise'"""
    np.random.seed(s)
    avg_abrocas = None
